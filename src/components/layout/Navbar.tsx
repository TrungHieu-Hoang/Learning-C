    ...
    <nav className="bg-mantle border-b border-surface0 sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/settings"
            className="text-overlay0 hover:text-text transition-colors p-2"
          >
      ...
      {/* Mobile bottom nav */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-mantle border-t border-surface0 z-40 flex justify-around py-2">
        {navItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
